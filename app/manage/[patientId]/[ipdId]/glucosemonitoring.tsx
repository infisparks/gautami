...
export default function GlucoseMonitoring() {
  ...
  const handleVoiceInput = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const structuredData: Partial<GlucoseFormInputs> = JSON.parse(
          result.candidates[0].content.parts[0].text
        );

        // Only overwrite fields that AI actually returned non-empty
  ...
  const onSubmit: SubmitHandler<GlucoseFormInputs> = async (data) => {
    setIsSubmitting(true);
    const newRef = push(
      ref(db, `patients/${patientId}/ipd/${ipdId}/glucoseReadings`)
    );
    await set(newRef, {
      ...getValues(),
